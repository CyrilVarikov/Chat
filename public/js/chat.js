class Chat{
    constructor(){
        ...
        this.socket.on('connect', () => {
            document.querySelector('#info').textContent = 'Get yourself a nickname';
        ...
        this.socket.on('nickExisted', () =>{
            document.querySelector('#info').textContent = 'Nickname is taken, choose another please';
        });
        ...
        this.socket.on('error', (err) => {
            if (document.querySelector('#loginWrapper').style.display == 'none') {
                document.querySelector('#status').textContent = 'Fail to connect';
            } else {
                document.querySelector('#info').textContent = 'Fail to connect';
            }
        });

        this.socket.on('system', (nickName, userCount, type) => {
            const msg = '@'+nickName + (type == 'login' ? ' joined' : ' left');
            that.displayNewMsg('System ', msg, '#C7050A');
            document.querySelector('#status').textContent = userCount + (userCount > 1 ? ' users' : ' user') + ' online';
        });
        ...
        document.querySelector('#sendImage').addEventListener('change', function() {
            if (this.files.length !== 0) {
                ...
                if (!reader) {
                    that.displayNewMsg('System', 'Your browser doesn\'t support fileReader', '#C7050A');
                    this.value = '';
                    return;
                }
    ...
    displayNewMsg(user, msg, color) {
        ...
        msgToDisplay.innerHTML = user + '<span class="timespan">(' + date + '): </span>' + msgN;
